...
# Instalar dependencias del sistema (INCLUYENDO SUDO)
RUN apt-get update && apt-get install -y \
    ...
    supervisor \
    # Utilidades
    openssl ca-certificates htop tree unzip jq \
    # Limpieza
    && apt-get clean \
...
# Configurar PostgreSQL SIN SUDO
...
RUN echo '#!/bin/bash' > /usr/local/bin/register_matrix_user \
    && echo 'su - matrix -c "/opt/matrix/env/bin/register_new_matrix_user -c /opt/matrix/homeserver.yaml http://localhost:8008 $*"' >> /usr/local/bin/register_matrix_user \
